...
(* \#line cholera-model.nw 2304 *)
val _ = new_theory "cholstatics";
local open cholexprTheory in end;
open Datatype listTheory

(* \#line cholera-model.nw 2560 *)
val cond_typing_conds = new_definition(
  "cond_typing_conds",
  ``cond_typing_conds (gty, t1, t2, restype) =
      scalar_type gty /\
      (arithmetic_type t1 /\ arithmetic_type t2 /\
       (restype = ua_conversions t1 t2) \/
      (?s. (t1 = Struct s) /\ (t1 = t2) /\ (t1 = restype)) \/
      pointer_type t1 /\ pointer_type t2 /\
      ((t1 = t2) /\ (restype = t1) \/
       Ptr Void IN {t1; t2} /\ (restype = Ptr Void)) \/
      (t1 = Void) /\ (t2 = t1) /\ (restype = t1))``);
(* \#line cholera-model.nw 2578 *)
val cond_typing_conds_det = store_thm(
  ...
  ``!gty t1 t2 rt1 rt2.
       ...
       (rt1 = rt2)``,
  SIMP_TAC hol_ss [NOT_IN_EMPTY, IN_INSERT, cond_typing_conds] THEN
  REPEAT STRIP_TAC THEN ELIM_TAC THEN ASM_SIMP_TAC hol_ss [] THEN
  ASM_MESON_TAC [choltypeTheory.type_classes]);
val cond_typing_wellformed = store_thm(
  ...
  ``!gty smap t1 t2 rt.
       ...
       well_formed_type smap rt``,
  SIMP_TAC hol_ss [cond_typing_conds] THEN REPEAT STRIP_TAC THEN
  ELIM_TAC THEN ASM_SIMP_TAC hol_ss [
    choltypeTheory.well_formed_type_THM,
    choltypeTheory.ua_converted_types_well_formed
  ]);
(* \#line cholera-model.nw 3025 *)
val arithmetic_pair_type = new_definition (
  "arithmetic_pair_type",
  ``arithmetic_pair_type t1 t2 t =
      arithmetic_type t1 /\ arithmetic_type t2 /\
      (t = ua_conversions t1 t2)``);
val binary_op_type = new_recursive_definition {
  name = "binary_op_type",
  rec_axiom = cholexprTheory.c_binops_Axiom,
  def = ``
    ...
    (binary_op_type CMod t1 t2 t =
      ...
      (t = ua_conversions t1 t2))``};
(* \#line cholera-model.nw 3074 *)
val unary_op_type = new_recursive_definition {
  name = "unary_op_type",
  rec_axiom = cholexprTheory.c_unops_Axiom,
  def = ``
...
val ops_dont_produce_arrays = store_thm(
  ...
  CONJ_TAC THEN GEN_TAC THENL [
    STRUCT_CASES_TAC (Q.SPEC `f` cholexprTheory.c_binops_nchotomy),
    STRUCT_CASES_TAC (Q.SPEC `f` cholexprTheory.c_unops_nchotomy)
  ] THEN SIMP_TAC hol_ss [binary_op_type, unary_op_type,
    choltypeTheory.array_type, choltypeTheory.CType_distinct,
    arithmetic_pair_type] THEN
  dMESON_TAC 10 [choltypeTheory.type_classes, choltypeTheory.integral_type,
                 choltypeTheory.integral_promotions_safe,
                 choltypeTheory.ua_conversions_safe]);
(* \#line cholera-model.nw 2322 *)
val _ = Hol_datatype `expr_value_type = LValue | RValue`;
(* \#line cholera-model.nw 2328 *)
local
    val e  = (--`INL:CExpr -> CExpr + CExpr list`--)
    val el = (--`INR:CExpr list -> CExpr + CExpr list`--)
    val t  = (--`INL:CType -> CType + CType list`--)
    val tl = (--`INR:CType list -> CType + CType list`--)
in
  val (expr_type_rules, expr_type_ind, expr_type_cases) =
  IndDefLib.Hol_reln`
      
(* \#line cholera-model.nw 2407 *)
(!s e t. ~array_type t /\ expr_type s LValue (^e e) (^t t) ==>
         expr_type s RValue (^e e) (^t t)) /\
(* \#line cholera-model.nw 2415 *)
(!s e bt n. expr_type s LValue (^e e) (^t (Array bt n)) ==>
            expr_type s RValue (^e e) (^t (Ptr bt))) /\
(* \#line cholera-model.nw 2422 *)
(!s n. expr_type s RValue (^e (Cnum n)) (^t (Signed Int)))
                /\
(!s c. expr_type s RValue (^e (Cchar c)) (^t (Signed Int)))
                /\
(!smap tmap t. well_formed_type smap t ==>
               expr_type (smap,tmap) RValue (^e (Cnullptr t)) (^t (Ptr t))) /\
(* \#line cholera-model.nw 2435 *)
(!smap tmap n.
...
(!smap tmap n t. well_formed_type smap t /\ ~(t = Void) ==>
                 expr_type (smap,tmap) LValue (^e (LVal n t)) (^t t)) /\
(* \#line cholera-model.nw 2451 *)
(!smap tmap v t. well_formed_type smap t /\ ~array_type t ==>
                 expr_type (smap,tmap) RValue (^e (ECompVal v t)) (^t t))
     /\
(!s e t.  expr_type s RValue (^e e) (^t t) ==>
          expr_type s RValue (^e (RValreq e)) (^t t)) /\
(* \#line cholera-model.nw 2466 *)
(!smap tmap e t t0.
    well_formed_type smap t /\ (scalar_type t \/ (t = Void)) /\
    expr_type (smap, tmap) RValue (^e e) (^t t0) ==>
    expr_type (smap,tmap) RValue (^e (Cast t e)) (^t t)) /\
(* \#line cholera-model.nw 2481 *)
(!env v t. expr_type env v (^e UndefinedExpr) (^t t))
(* \#line cholera-model.nw 2336 *)
                                         /\
      
(* \#line cholera-model.nw 2494 *)
(!s t n args rt.  well_formed_type s (t n) /\
                  (t n = Function rt args) ==>
                  expr_type (s,t) RValue (^e (CFunRef n)) (^t (t n))) /\
