...
\item[The heap] This is handled only in very rudimentary fashion.
  ...
  \textttt{delete} properly will require the state to record the types
  of what was allocated.  (When a call to \texttt{delete []} is made,
  the state needs to know how big the array was.) The allocation and
  of heap memory is not challenging.  Handling placement-\texttt{new}
  is likely to be more intricate to specify.
