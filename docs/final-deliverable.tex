...
\end{verbatim}
  \caption[A Template Program that Breaks Siek and Taha's Model]{Siek
    ...
    the definition of \texttt{Baz<A>::g}.  At this point, it can not
    tell which template is being instantiated in the body.  By way of
...
\begin{verbatim}
  ...
  VDecInitA int      (ObjPlace (a + szoff))
                     (CopyInit (NormE (FnApp (Var "strlen")
                                      ...
                                      base_se))
\end{verbatim}
  \caption[Generated Variable Declarations from
...
they are of less interest) are: enumerated types, unions, bit-field
members and the \texttt{goto} and \texttt{switch} statements.  Also,
for example, while I provide a rule for post-increment \texttt{++},
there is no rule for \texttt{--}.
