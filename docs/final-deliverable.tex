...
Thus the rule for taking an address:
...
\end{alltt}
\index{LVal@\texttt{LVal}}
The function \texttt{static_type} computes the static type for a
l-value given access to the type and path of identifiers.  (For
non-class values, the static type is simply the second argument to
\texttt{LVal}.)
...
in C.  Nonetheless, this semantics adopts Clive Feather's
proposal~\cite{Feather2000} for handling the infamous language in the
standard: 
\begin{quotation}
\itshape
...
context (\ruleid{expr-econtext}, p\pageref{rule:econtext-expr}) can
...
\index{LVal constructor, support for dynamic types@\texttt{LVal} constructor, support for dynamic types}
%
...
The \ruleid{virtual-function-member-select} rule governs the evaluation of
...
\index{class_part@\texttt{class_part}} The first hypothesis is
