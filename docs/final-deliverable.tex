...
typed term language.  In higher-order logic, unlike in predicate
logic, there is no distinction between terms and formulas.  The latter
are simply terms of type \texttt{:bool}.
...
\begin{alltt}
   xty # yty -> result\_ty
\end{alltt}
(where \texttt{\#} indicates the cartesian product).  In the second
term, the type of \texttt{f} is
\begin{alltt}
   xty -> (yty -> result\_ty)
\end{alltt}
